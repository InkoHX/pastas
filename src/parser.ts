import { pastasRegExp, PastasRegExpOptionKeyOnlyGroup, PastasRegExpOptionQuoteGroup } from "./utils"
...
export type CustomTypeParserProps = CustomTypeParserFlagProps | CustomTypeParserArgumentProps
export type CustomTypeParser<T = any> = (param: Readonly<CustomTypeParserProps>) => T

type Argument<T extends ReadonlyArray<CustomTypeParser>> = {
  [K in keyof T]: T[K] extends CustomTypeParser<infer R>
  ? R
  : never
}
...
export function pastas<A extends ReadonlyArray<CustomTypeParser>>(source: string, args: A): [Argument<A>]
export function pastas<
  A extends ReadonlyArray<CustomTypeParser>,
  O extends Record<string, CustomTypeParser | keyof O>
>(source: string, args: A, options: O): [Argument<A>, Pick<Option<O>, OmitOptionAliasKeys<O>>]
export function pastas<
  A extends ReadonlyArray<CustomTypeParser>,
  O extends Record<string, CustomTypeParser | keyof O>
>(
  ...
  const parsedArgs = source
    ...
    .map((sourceArg, index) => typeof args[index] === 'function' ? (args[index] as CustomTypeParser)({ type: 'argument', value: sourceArg, position: index }) : IGNORE_ARGUMENT)
  ...
  const replacedAlias: { [key: string]: CustomTypeParser | typeof UNKNOWN_ALIAS } = {
    ...options,
    ...Object.fromEntries(
      entriesOptions
  ...
  for (const iter of withValueOptions) {
    for (const { groups } of iter) {
      if (!groups) continue
      const { name, value } = groups as unknown as PastasRegExpOptionQuoteGroup
      const originalOptionName = (entriesOptions.find(([_key, value]) => value === name))?.[0] ?? name
      ...
      if (optionParser === UNKNOWN_ALIAS) throw new Error('不明なエイリアスが割り当てられています。')

      const result = optionParser({ type: 'option', name: originalOptionName, value })
  ...
  return [parsedArgs, Object.fromEntries(parsedOptions) as Pick<Option<O>, OmitOptionAliasKeys<O>>]
}
